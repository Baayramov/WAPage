...
bool DatabaseManager::createTables()
{
    ...
    q.exec("ALTER TABLE participants ADD COLUMN message_count INTEGER DEFAULT 0");
    
    // Lock system columns for old DBs
    q.exec("ALTER TABLE chats ADD COLUMN is_locked INTEGER DEFAULT 0");
    q.exec("ALTER TABLE settings ADD COLUMN master_password TEXT");
...
{
    return allChats(false); // Default: only unlocked chats
}

QList<Chat> DatabaseManager::allChats(bool includeLockedChats)
{
    QList<Chat> chats;
    QSqlQuery q(m_db);
    
    QString whereClause = includeLockedChats ? "" : "WHERE c.is_locked=0";
    
    QString queryStr = QString(R"(
        SELECT c.id, c.contact_name, c.chat_type, c.zip_source_path,
               c.custom_avatar_path, c.imported_at, c.last_message_time, c.is_locked,
    ...
    while (q.next()) {
        ...
        ch.isLocked          = q.value(7).toBool();
        ch.lastMessagePreview= q.value(8).toString();
        
        // PROFILE LIBRARY INTEGRATION: Check for centralized avatar
        if (!ch.contactName.trimmed().isEmpty()) {
            ...
            // For GROUP chats: Use name as-is
            
            if (!chatName.isEmpty()) {
                QString libraryAvatar = getProfileLibraryAvatar(chatName);
                if (!libraryAvatar.isEmpty()) {
                    ch.customAvatarPath = libraryAvatar;
                }
            }
        }
...
Chat DatabaseManager::getChatById(int chatId)
{
    Chat ch;
    if (chatId < 0) {
        return ch;  // Return empty chat
    }
...
QList<Participant> DatabaseManager::participantsForChat(int chatId)
{
    ...
    QSqlQuery q(m_db);
    q.prepare(R"(
        SELECT id, chat_id, participant_name, custom_avatar_path, message_count
        FROM participants
        WHERE chat_id=:cid
    ...
    while (q.next()) {
        ...
        p.messageCount = q.value(4).toInt();  // Load cached count
...
{
    // Use pre-computed cached count from participants.message_count
    // This cache is populated during import by computeAndCacheParticipantMessageCounts()
    // For old chats (imported before this feature), returns 0 instead of computing live
    QSqlQuery q(m_db);
    q.prepare(R"(
    ...
    q.bindValue(":cid", chatId);
    q.bindValue(":name", participantName);
    
    if (q.exec() && q.next()) {
        return q.value(0).toInt();  // Returns cached count (or 0 if not cached)
    }
    
    // Participant not found in database - return 0
    return 0;
}
...
bool DatabaseManager::computeAndCacheParticipantMessageCounts(int chatId)
{
    if (chatId < 0) {
        return false;
    }

    Logger::info(QString("[MSG-COUNT-CACHE] Computing message counts for chat_id=%1").arg(chatId).toStdString());

    // Query to count messages per sender, handling tilda variants
    // This counts ALL sender name variants (with/without tilda) as ONE participant
    QSqlQuery q(m_db);
    q.prepare(R"(
        SELECT 
            CASE 
                WHEN sender LIKE '~ %' THEN TRIM(SUBSTR(sender, 3))
                WHEN sender LIKE '~%' THEN TRIM(SUBSTR(sender, 2))
                ELSE sender 
            END AS normalized_sender,
            COUNT(*) as msg_count
        FROM messages
        WHERE chat_id = :cid
        GROUP BY normalized_sender
    )");
    q.bindValue(":cid", chatId);
    
    if (!q.exec()) {
        Logger::error("[MSG-COUNT-CACHE] Query error: " + q.lastError().text().toStdString());
        return false;
    }

    // Collect counts
    QHash<QString, int> senderCounts;
    while (q.next()) {
        const QString sender = q.value(0).toString().trimmed();
        const int count = q.value(1).toInt();
        if (!sender.isEmpty() && count > 0) {
            senderCounts[sender] = count;
        }
    }

    if (senderCounts.isEmpty()) {
        Logger::info("[MSG-COUNT-CACHE] No messages found for this chat");
        return true;
    }

    Logger::info(QString("[MSG-COUNT-CACHE] Found %1 participants with messages").arg(senderCounts.size()).toStdString());

    // Update participants table with cached counts
    QSqlQuery updateQ(m_db);
    updateQ.prepare(R"(
        UPDATE participants 
    ...
    int updated = 0;
    for (auto it = senderCounts.constBegin(); it != senderCounts.constEnd(); ++it) {
        updateQ.bindValue(":count", it.value());
        updateQ.bindValue(":cid", chatId);
        updateQ.bindValue(":name", it.key());
        
        if (updateQ.exec()) {
            updated++;
        } else {
            Logger::warn(QString("[MSG-COUNT-CACHE] Failed to update '%1': %2")
                .arg(it.key()).arg(updateQ.lastError().text()).toStdString());
        }
    }

    Logger::info(QString("[MSG-COUNT-CACHE] Successfully cached counts for %1/%2 participants")
        .arg(updated).arg(senderCounts.size()).toStdString());
...
{
    return profileExistsInLibrary(name, false); // Default: check all profiles
}

bool DatabaseManager::profileExistsInLibrary(const QString& name, bool onlyCustom)
{
    const QString normalizedName = normalizeString(name);
    if (normalizedName.isEmpty()) {
        return false;
    ...
    QSqlQuery q(m_db);
    if (onlyCustom) {
        // Only check CUSTOM profiles (is_default=0), ignore auto-generated defaults
        q.prepare("SELECT COUNT(*) FROM profile_library WHERE name=:name AND (is_default=0 OR is_default IS NULL)");
    } else {
        // Check ALL profiles (both custom and default)
...
{
    setSetting("master_password", password);
    Logger::info("Master password updated");
    return true;
}

QString DatabaseManager::getMasterPassword()
{
    return getSetting("master_password", "");
}

bool DatabaseManager::hasMasterPassword()
{
    QString pw = getMasterPassword();
    return !pw.trimmed().isEmpty();
}

bool DatabaseManager::lockChat(int chatId)
{
    if (chatId < 0) {
        return false;
    }
    ...
    QSqlQuery q(m_db);
    q.prepare("UPDATE chats SET is_locked=1 WHERE id=:id");
    q.bindValue(":id", chatId);
    bool ok = q.exec();
    
    if (ok) {
        Logger::info(QString("Chat %1 locked").arg(chatId).toStdString());
    } else {
        Logger::error("Lock chat error: " + q.lastError().text().toStdString());
    }
    
    return ok;
}

bool DatabaseManager::unlockChat(int chatId)
{
    if (chatId < 0) {
        return false;
    }
    ...
    QSqlQuery q(m_db);
    q.prepare("UPDATE chats SET is_locked=0 WHERE id=:id");
    q.bindValue(":id", chatId);
    bool ok = q.exec();
    
    if (ok) {
        Logger::info(QString("Chat %1 unlocked").arg(chatId).toStdString());
    } else {
        Logger::error("Unlock chat error: " + q.lastError().text().toStdString());
    }
    
    return ok;
}

bool DatabaseManager::isChatLocked(int chatId)
{
    if (chatId < 0) {
        return false;
    }
    ...
    QSqlQuery q(m_db);
    q.prepare("SELECT is_locked FROM chats WHERE id=:id");
    q.bindValue(":id", chatId);
...
QList<Chat> DatabaseManager::getLockedChats()
{
    QList<Chat> chats;
    QSqlQuery q(m_db);
    q.exec(R"(
