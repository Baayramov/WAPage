...
QVariant MessageListModel::data(const QModelIndex& index, int role) const
{
    ...
    case SenderAvatarRole: {
        QString avatar = m_senderAvatars.value(m.sender);
        // Log for debugging iOS group chat avatar issues - DETAILED
        if (avatar.isEmpty()) {
            qDebug() << "[SENDER-AVATAR] ❌ LOOKUP FAILED";
            qDebug() << "[SENDER-AVATAR]   Searched for:" << m.sender;
            qDebug() << "[SENDER-AVATAR]   Map has" << m_senderAvatars.keys().size() << "keys";
            qDebug() << "[SENDER-AVATAR]   Available keys:" << m_senderAvatars.keys();
        } else {
            qDebug() << "[SENDER-AVATAR] ✅ Found avatar for:" << m.sender << "=>" << avatar;
        }
        return avatar;
    }
...
void MessageListModel::setSenderAvatarMap(const QHash<QString, QString>& avatarsBySender)
{
    ...
    qDebug() << "[SENDER-AVATAR-MAP] ========== setSenderAvatarMap called ==========";
    qDebug() << "[SENDER-AVATAR-MAP] Map has" << avatarsBySender.size() << "entries";
    qDebug() << "[SENDER-AVATAR-MAP] Messages count:" << m_messages.size();
    
    if (!avatarsBySender.isEmpty()) {
        qDebug() << "[SENDER-AVATAR-MAP] ALL keys in map:" << avatarsBySender.keys();
        // Log a few values
        int count = 0;
        for (auto it = avatarsBySender.constBegin(); it != avatarsBySender.constEnd() && count < 5; ++it, ++count) {
            qDebug() << "[SENDER-AVATAR-MAP]   " << it.key() << "=>" << it.value();
        }
    }
    
    if (!m_messages.isEmpty()) {
        qDebug() << "[SENDER-AVATAR-MAP] Emitting dataChanged for" << m_messages.size() << "messages";
        emit dataChanged(index(0), index(m_messages.size() - 1), {SenderAvatarRole});
        qDebug() << "[SENDER-AVATAR-MAP] dataChanged emitted successfully";
    } else {
        qDebug() << "[SENDER-AVATAR-MAP] ⚠️  No messages to update!";
    }
    qDebug() << "[SENDER-AVATAR-MAP] ========== END ==========";
}
