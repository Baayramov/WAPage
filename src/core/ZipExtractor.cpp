...
static bool runExtractZip(const QString& archivePath,
...
#else
    // Linux / macOS: use bsdtar (part of libarchive, standard on Arch/CachyOS)
    // It handles Windows filename encodings (mojibake) much better than unzip.
...
#else
        errorOut = "ZIP extraction could not start (unzip not found — install with: sudo apt install unzip).";
#endif
        return false;
    }
    ...
    const std::function<void(const ExtractProgress&)>& progressCallback)
{
    ...
    auto runExpandArchiveFallback = [&](QString& errorOut) {
        ...
        const bool ok = runExtractZip(zipPath, outDir, kExtractTimeoutMs, errorOut);
        if (ok) {
            emitProgress(0.96,
                         "extract",
                         1,
