                                                                ...
                                                                int totalItems)>& progressCallback)
{
    ...
    for (auto& msg : messages) {
        ...
        if (fileName.isEmpty()) {
            ...
            msg.waveform.clear();
            msg.mediaMeta = "omitted";  // Mark as omitted when filename is invalid
            notFound++;
            processedMediaItems++;
            emitProgress();
            continue;
        ...
        if (srcPath.isEmpty() || !QFile::exists(srcPath)) {
            ...
            msg.mediaThumbnailPath.clear();
            msg.waveform.clear();
            msg.mediaMeta = "omitted";  // Mark as omitted when file not found
            notFound++;
            processedMediaItems++;
            emitProgress();
            continue;
        ...
        if (QFile::copy(srcPath, dstPath)) {
            ...
            msg.mediaThumbnailPath.clear();
            msg.waveform.clear();
            msg.mediaMeta = "omitted";  // Mark as omitted when copy fails
        }
