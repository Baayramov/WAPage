...

{
    ...
    // The returned QFuture is intentionally discarded — completion is signalled
    // back to the UI thread via QMetaObject::invokeMethod inside the lambda,
    // not via the future. The (void) cast silences MSVC C4858.

    (void)QtConcurrent::run([guard, token, localPath, maybeAnimated]() {
