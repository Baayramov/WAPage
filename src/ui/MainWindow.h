...
public:
    ...
    bool    activeChatIsGroup()  const
    {
        ...
        if (m_db) {
            ...
            if (chat.id > 0) {
                return chat.chatType.compare("group", Qt::CaseInsensitive) == 0;
            }
        }
    ...
    // Check audio file validity (exists, non-zero size) before loading
    Q_INVOKABLE QVariantMap checkAudioFile(const QString& filePath) const;
    ...
    Q_INVOKABLE void setParticipantAvatar(int chatId, const QString& participantName, const QString& imagePath)
    {
        ...
        // Strip tilde prefix for profile library
        QString nameForLibrary = trimmedName;
        if (nameForLibrary.startsWith("~ ")) {
            nameForLibrary = nameForLibrary.mid(2).trimmed();
        } else if (nameForLibrary.startsWith('~')) {
            nameForLibrary = nameForLibrary.mid(1).trimmed();
        }
        
        if (!nameForLibrary.isEmpty()) {
            // AUTO-SYNC: Add/update participant in Profile Library
            if (m_db->profileExistsInLibrary(nameForLibrary)) {
                m_db->updateProfileLibraryAvatar(nameForLibrary, avatarPath);
            } else {
                m_db->addProfileToLibrary(nameForLibrary, avatarPath, false); // false = custom avatar
            }
            
            // Propagate avatar change EVERYWHERE
            syncAvatarEverywhere(nameForLibrary, avatarPath);
    ...
    Q_INVOKABLE bool hasMasterPassword();
    Q_INVOKABLE bool checkMasterPassword(const QString& password);
    Q_INVOKABLE bool lockChat(int chatId);
    Q_INVOKABLE bool unlockChat(int chatId);
    Q_INVOKABLE bool isChatLocked(int chatId);
    Q_INVOKABLE QVariantList getLockedChats(); // Returns list of locked chats
    Q_INVOKABLE void refreshChatList(); // Reload chat list (after lock/unlock)
...
signals:
    ...
    void lockedChatsChanged(); // Emitted when chats are locked/unlocked
...
private:
    ...
    void reloadActiveParticipants()
    {
        ...
        for (Participant& participant : participants) {
            ...
            // Use pre-cached message count from Participant struct (no extra query needed)
            row.insert("messageCount", participant.messageCount);
            next.append(row);
        }
