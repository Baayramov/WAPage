...
Item {
    ...
    // Signal: emitted when a voice message finishes playback
    signal voiceMessageFinished(int messageId, real playbackRate)
    
    // Method to trigger audio playback (for auto-play feature)
    // rate parameter: optionally set playback rate before playing
    function triggerPlay(rate) {
        console.log("🎬 MessageBubble.triggerPlay: msgType=" + msgType + " msgId=" + msgId + " rate=" + (rate || "not provided"))
        
        if (msgType !== "ptt" && msgType !== "audio") {
            console.log("  ⏭️  SKIP: not audio/ptt")
            return
        }
        
        // Wait for contentLoader to be ready if needed
        Qt.callLater(function() {
            if (!contentLoader || !contentLoader.item) {
                console.log("  ❌ ERROR: contentLoader or item is null")
                return
            }
            
            const audioPlayer = contentLoader.item
            
            if (typeof audioPlayer.play !== "function") {
                console.log("  ❌ ERROR: play function not found")
                return
            }
            
            // Save current position to resume from where it left off
            const savedPosition = (audioPlayer.smoothPosition !== undefined && audioPlayer.smoothPosition > 0) 
                                ? audioPlayer.smoothPosition 
                                : 0
            console.log("  💾 savedPosition=" + savedPosition + "ms")
            
            // Set playback rate if provided - AudioPlayer will handle the speed change
            if (typeof rate === "number" && rate > 0) {
                console.log("  🎚️  Setting speedActivated=true, playbackRate=" + rate)
                audioPlayer.speedActivated = true
                audioPlayer.playbackRate = rate
            }
            
            // Resume from saved position and play
            // AudioPlayer's internal mechanism (_cycleSpeed) will handle cached files
            Qt.callLater(function() {
                if (savedPosition > 0) {
                    console.log("  📍 Resuming from position=" + savedPosition + "ms")
                    audioPlayer.smoothPosition = savedPosition
                }
    ...
    // Computed property: Is this sticker omitted/unavailable?
    // Sticker is considered omitted if:
    // 1. mediaMeta is explicitly "omitted" OR
    // 2. mediaPath is empty/missing (file not found)
    readonly property bool isStickerOmitted: 
        msgType === "sticker" && (mediaMeta === "omitted" || mediaPath.length === 0)
    
    readonly property bool showSenderHeader:
        isGroupChat && !isFromMe && isFirstInGroup && !isSys && !(msgType === "sticker" && !isStickerOmitted)
    ...
    height: {
        ...
        if (isSys) {
            ...
            else if (msgType === "audio" || msgType === "ptt")   typeH = (mediaMeta === "omitted" ? 70  : 64)
            else if (msgType === "sticker")                      typeH = (isStickerOmitted ? 70  : 176)
    ...
    readonly property int fixedBubbleHeight: {
        ...
        if (msgType === "audio" || msgType === "ptt")        return (mediaMeta === "omitted" ? 70  : 64) + senderExtra
        if (msgType === "sticker")                           return (isStickerOmitted ? 70  : 176) + (isStickerOmitted ? senderExtra : 0)
    ...
    readonly property color bubbleFillColor:
        (root.msgType === "sticker" && !root.isStickerOmitted)
            ? "transparent"
    ...
    Rectangle {
        ...
        width: {
            ...
            // Calculate dimensions without binding to child elements
            const maxWidth = parent.width * 0.75
            
            // Content width
            const contentWidth = (contentLoader.item ? contentLoader.item.implicitWidth : 0) + WAPageTheme.bubblePaddingH * 2
            
            // Meta row width
            const metaWidth = metaRow.width + WAPageTheme.bubblePaddingH * 2 + 4
            
            // For sender name, use a reasonable minimum instead of binding to label
            const senderMinWidth = root.showSenderHeader ? 150 : 0
        ...
        Rectangle {
            id: tailCornerPatch
            visible: root.isFirstInGroup && !(root.msgType === "sticker" && !root.isStickerOmitted)
        ...
        Loader {
            ...
            sourceComponent: {
                ...
                // Sticker: use omitted component if it's considered omitted
                if (root.msgType === "sticker")  return (root.isStickerOmitted ? omittedStickerComp : stickerComp)
        ...
        Row {
            ...
            visible: {
                ...
                if (root.msgType === "sticker" && !root.isStickerOmitted) return false
                return true
            }
            anchors {
    ...
    MouseArea {
        ...
        propagateComposedEvents: true  // Allow left clicks to pass through to child elements
        preventStealing: false          // Don't steal events from children
        ...
        onPressed: function(mouse) {
            mouse.accepted = (mouse.button === Qt.RightButton && !root.isSys)
        }
    }
    ...
    Menu {
        ...
        // Refresh favorite status when menu opens
        onAboutToShow: {
            favoriteMenuItem.refreshStatus()
        }
        
        MenuItem {
            id: favoriteMenuItem
            
            // Dynamic property to track current favorite status
            property bool currentFavoriteStatus: false
            
            function refreshStatus() {
                currentFavoriteStatus = mainWindow.isMessageFavorite(mainWindow.activeChatId, root.msgId)
            }
            
            // Initialize on component creation
            Component.onCompleted: refreshStatus()
            
            text: currentFavoriteStatus
                ? (langManager.currentTranslations["remove_from_favorites"] || "Favorilərədən sil")
                : (langManager.currentTranslations["add_to_favorites"] || "Favorilərə əlavə et")
            ...
            contentItem: Item {
                ...
                Row {
                    ...
                    Text {
                        ...
                        text: favoriteMenuItem.currentFavoriteStatus
                            ? (langManager.currentTranslations["remove_from_favorites"] || "Favorilərədən sil")
                            : (langManager.currentTranslations["add_to_favorites"] || "Favorilərə əlavə et")
            ...
            background: Rectangle {
                radius: 0
                color: favoriteMenuItem.highlighted 
                    ? (WAPageTheme.dark ? "#2A2A2A" : WAPageTheme.chatListHover) 
                    : (WAPageTheme.dark ? "#1C1C1C" : WAPageTheme.panelBg)
            }
            
            onTriggered: {
                if (currentFavoriteStatus) {
                    mainWindow.removeFromFavorites(mainWindow.activeChatId, root.msgId)
                } else {
                    mainWindow.addToFavorites(mainWindow.activeChatId, root.msgId)
                }
        ...
        // Listen to favoritesChanged signal to refresh when favorites change
        Connections {
            target: mainWindow
    ...
    Component {
        ...
        AudioPlayer {
            ...
            onPlaybackFinished: {
                console.log("MessageBubble: playbackFinished received, msgType=" + root.msgType + " msgId=" + root.msgId + " playbackRate=" + playbackRate)
                // Only emit signal for voice messages (PTT), not regular audio
                if (root.msgType === "ptt") {
                    console.log("MessageBubble: emitting voiceMessageFinished for msgId=" + root.msgId)
                    root.voiceMessageFinished(root.msgId, playbackRate)
                }
            }
        }
    }
