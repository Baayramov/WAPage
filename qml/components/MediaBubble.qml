...
Item {
    ...
    // Internal property to hold actual video duration
    property string _actualVideoDuration: ""

    implicitWidth:  240
    implicitHeight: 200
    
    // Hidden MediaPlayer to extract video duration
    MediaPlayer {
        id: videoDurationExtractor
        audioOutput: AudioOutput { volume: 0 }
        
        onDurationChanged: {
            if (root.mediaType === "video" && duration > 0) {
                const totalSec = Math.floor(duration / 1000)
                const mins = Math.floor(totalSec / 60)
                const secs = totalSec % 60
                root._actualVideoDuration = mins + ":" + (secs < 10 ? "0" : "") + secs
            }
        }
        
        Component.onCompleted: {
            if (root.mediaType === "video" && root.mediaPath.length > 0) {
                source = "file:///" + root.mediaPath.replace(/\\/g, "/")
            }
        }
    }
    
    // Update source when mediaPath changes
    onMediaPathChanged: {
        if (root.mediaType === "video" && root.mediaPath.length > 0) {
            videoDurationExtractor.source = "file:///" + root.mediaPath.replace(/\\/g, "/")
        }
    }
    ...
    Rectangle {
        ...
        Rectangle {
            visible: root.mediaType === "video" && (root._actualVideoDuration.length > 0 || root.mediaDuration.length > 0)
            anchors {
                left:         parent.left
            ...
            Text {
                id: durationText
                anchors.centerIn: parent
                text:           root._actualVideoDuration.length > 0 ? root._actualVideoDuration : root.mediaDuration
                color:          "#FFFFFF"
                font.family:    WAPageTheme.fontFamily
                font.pixelSize: WAPageTheme.fontSizeMicro
            }
