...
Rectangle {
    ...
    // When step changes to 2, pre-fill with default owner name if available
    onStepChanged: {
        if (step === 2 && ownerName.length === 0 && mainWindow.defaultOwnerName.length > 0) {
            ownerName = mainWindow.defaultOwnerName
            ownerInput.text = mainWindow.defaultOwnerName
        }
    }
    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            ColumnLayout {
                ...
                // Show default owner name if set
                Text {
                    visible: mainWindow.defaultOwnerName.length > 0
                    Layout.fillWidth: true
                    text: (langManager.currentTranslations["default_label"] || "Default") + ": " + mainWindow.defaultOwnerName
                ...
                Rectangle {
                    ...
                    // Auto-focus when step becomes 2 (for drag & drop)
                    Timer {
                        id: focusTimer
                        interval: 100
                        repeat: false
                        onTriggered: ownerInput.forceActiveFocus()
                    }
