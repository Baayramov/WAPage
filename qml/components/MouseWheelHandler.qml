...
Item {
    ...
    property bool handlerEnabled: true       // disabled until contentReady
    ...
    // Reset offset when handlerEnabled changes
    onHandlerEnabledChanged: {
        if (!handlerEnabled) {
            yOffset = 0
            returnTimer.stop()
        }
    ...
    MouseArea {
        ...
        onWheel: (wheel) => {
            if (!root.target || !root.handlerEnabled) {
