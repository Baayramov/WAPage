...
Item {
    ...
    function _cycleSpeed() {
        ...
        // smoothPosition is more reliable than player.position right after a
        // swap (player.position can momentarily be 0 while reloading).
        const posMs       = (player.position > 0) ? player.position
                                                   : root.smoothPosition
    ...
    function _applyPendingSeek() {
        if (root._pendingSeekMs < 0) return
        if (player.duration <= 0) return
    ...
    Item {
        ...
        Item {
            ...
            Item {
                id: avatarLayer
                anchors.fill: parent
                opacity: root.speedActivated ? 0.0 : 1.0
            ...
            // ── Speed badge layer (after first tap) ─────────────────────
            Rectangle {
                id: speedBadge
                ...
                opacity: root.speedActivated ? 1.0 : 0.0
                visible: opacity > 0.01
                scale:   root.speedActivated ? 1.0 : 0.6
                Behavior on opacity { NumberAnimation { duration: 220; easing.type: Easing.OutCubic } }
