...
Item {
    ...
    function _applySourceUrl(newUrl) {
        ...
        // Safety check: validate the file exists and has valid size before loading
        // This prevents FFmpeg ASSERT crashes on corrupt/incomplete audio files
        if (newUrl.startsWith("file:///")) {
            var filePath = newUrl.substring(8) // Remove "file:///"
            
            // Check if file exists and has reasonable size
            if (typeof mainWindow !== "undefined") {
                var fileInfo = mainWindow.checkAudioFile(filePath)
                if (!fileInfo.exists) {
                ...
                if (fileInfo.size === 0) {
                    console.error("❌ Audio file is empty (0 bytes): " + filePath)
                    player.stop()
                    player.source = ""
                    return
                }
                if (fileInfo.size < 100) {
                    console.error("❌ Audio file is too small (corrupt): " + filePath + " size=" + fileInfo.size)
                    player.stop()
                    player.source = ""
                    return
                }
    ...
    MediaPlayer {
        ...
        onMediaStatusChanged: function() {
            ...
            // Handle invalid/corrupt media gracefully
            if (player.mediaStatus === MediaPlayer.InvalidMedia) {
                console.error("    ❌ InvalidMedia detected - audio file is corrupt or missing!")
                console.error("    source=" + player.source)
        ...
        onErrorOccurred: function(error, errorString) {
            ...
            // Don't let it crash the app - just log and continue
        }
    }
